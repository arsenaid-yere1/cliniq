...
### R4 — Capture reading **context** on every pain entry
**Problem:** #3. **Migration:** add `pain_context enum('at_rest','with_activity','worst_24h','current')`, `pain_reading_phase enum('pre_procedure','post_procedure','follow_up')` to `vital_signs` and `discharge_notes`. Update `prp-procedure.ts` validator + record-procedure dialog + discharge vitals card. Render in the prompt so the LLM doesn't conflate a post-procedure 2/10 reading with a pre-procedure baseline. Default to `'current'` / `'pre_procedure'` for back-compat.
**Files:** new migration, [prp-procedure.ts](src/lib/validations/prp-procedure.ts), [record-procedure-dialog.tsx](src/components/procedures/record-procedure-dialog.tsx), [discharge-note-editor.tsx](src/components/discharge/discharge-note-editor.tsx).
