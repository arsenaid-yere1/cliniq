...
3. **`generateDischargeNote(caseId)`** — Main generation flow:
   ...
   - Prerequisite check
   - Gather source data
   - Soft-delete existing discharge note for case
...
5. **`saveDischargeNote(caseId, values)`** — Save draft edits (validates with `dischargeNoteEditSchema`)
...
Follow the pattern of `render-procedure-note-pdf.ts`:
...
- Construct `DischargeNotePdfData` object
...
- `draft` → Section-by-section editor with:
  ...
  - "Save Draft" button (calls `saveDischargeNote`)
