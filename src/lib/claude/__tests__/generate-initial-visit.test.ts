...
describe('generateInitialVisitFromData', () => {
  ...
  it('calls callClaudeTool with Opus 4.6 and the initial visit tool', async () => {
...
describe('NUMERIC-ANCHOR for pain evaluation visit', () => {
  ...
  it('requires structured target selections in the tool contract', async () => {
    ...
    expect(opts.tools[0].input_schema.properties.prp_target_recommendations.items.properties.guidance_method.enum)
      .toEqual(['ultrasound'])
  })
