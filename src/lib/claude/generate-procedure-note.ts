...
• "improved" — the patient's pain has meaningfully decreased (≥3 points on the 0-10 scale) since the most recent injection. Describe this as improvement or reduced pain; remaining symptoms should be characterized as residual, intermittent, or mild where supported by the data. Do NOT use the words "persistent" or "continues to report" in this branch. Reference the prior visit's pain_score_max explicitly.
• "stable" — the patient's pain has changed modestly or not at all (delta in [-2, +1]). Describe symptoms as largely unchanged, persistent at a similar level, or modestly altered. Reference the prior visit's pain_score_max explicitly.
...
• "stable" (delta in [-2, +1]) — describe findings as largely unchanged from the prior injection; you may use "persistent at a similar level" or "without meaningful interval change" framing. Do not artificially soften or harden findings.
