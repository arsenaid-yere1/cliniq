...
• ANTI-DRIFT (MANDATORY): under NO circumstances substitute a "more clinically plausible" pain number for \`dischargeVisitPainDisplay\`. Do not round, do not soften, do not split the range, do not compromise between the estimated endpoint and the latest procedure reading. If the estimated endpoint reads "2-3/10" you write "2-3/10" — even if it feels low for a discharge after PRP series. The estimate is a documented audit anchor; clinical-judgment substitution silently desynchronizes the prose from the deterministic trajectory. The validator will flag every section that emits any \`X/10\` pattern not present in {painTrajectoryText, baselinePainDisplay, intakePainDisplay, firstProcedurePainDisplay, dischargeVisitPainDisplay, every procedure pain_score_max range from \`procedures\`}. If a number you want to write does not appear in that set, you are about to drift — stop and render \`dischargeVisitPainDisplay\` verbatim instead.
