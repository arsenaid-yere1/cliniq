...
RULES:
• The discharge follow-up visit is a separate encounter occurring AFTER the final injection. Patients reliably report continued improvement between the final PRP injection and this follow-up. You MUST render a discharge-visit pain reading that is 1-2 points BELOW \`latestVitals.pain_score_max\`, floored at 0. Example: last procedure pain 3-4/10 → discharge visit 2/10 or 2-3/10. Last procedure pain 2/10 → discharge visit 1/10. Last procedure pain 0-1/10 → discharge visit 0/10. This is the ONE exception to the "don't invent numbers" rule — it reflects expected post-injection healing and is scoped strictly to the discharge-visit pain endpoint.
• In \`subjective\`, you MUST explicitly narrate the downward trajectory ending at today's discharge reading (not at the last procedure). Format: "pain has decreased from X/10 at the initial evaluation to Z/10 at today's discharge visit", where Z is 1-2 points below the last procedure's pain_score_max. When more than 2 procedures exist, render the full series and terminate at the discharge reading (e.g., "8/10 → 6/10 → 4/10 → 2/10 across the injection series, and has further improved to 1/10 at today's discharge evaluation").
• Render pain ranges when min/max differ (e.g., "6-7/10"); single value when they match or only one is present.
• In \`objective_vitals\`, the Pain bullet reflects the discharge-visit reading (1-2 points below \`latestVitals.pain_score_max\`), NOT \`latestVitals\` directly.
...
• If "overallPainTrend" is "stable" or "worsened", DO NOT fabricate improvement — the small post-procedure delta rule does NOT apply. Describe the actual course honestly. This should be rare for a discharge note; if source data shows no improvement, write the narrative truthfully rather than forcing an optimistic framing.
...
Post-PRP follow-up narrative. Describe the patient's self-reported improvement since completing PRP treatment. MUST include the explicit pain trajectory per the PAIN TRAJECTORY rules above, ending at the discharge-visit reading (1-2 points below last procedure's pain_score_max).
...
Para 2: Region-by-region symptom status — current pain rating at discharge, quality of remaining pain (mild stiffness vs sharp), improvement in mobility, resolution of radicular symptoms. REQUIRED: cite the pain trajectory using concrete numbers from baselinePain → each procedure → discharge-visit reading (e.g., "pain has decreased from 7-8/10 at the initial evaluation to 1-2/10 at today's discharge visit" — where last procedure pain was 3/10, discharge shows continued improvement to 1-2/10). When procedures[] has 3+ entries, render the full series terminating at the discharge reading (e.g., "8/10 → 6/10 → 4/10 → 2/10 across the injection series, and has further improved to 1/10 at today's discharge evaluation").
...
BP, HR, RR, Temp, SpO2, Pain. Use \`latestVitals\` (the most recent procedure's vitals) for BP/HR/RR/Temp/SpO2. Pain bullet is the DISCHARGE-VISIT reading: 1-2 points below \`latestVitals.pain_score_max\`, floored at 0. Render as "• Pain: X-Y/10" when rendering a range (e.g., last procedure 3-4/10 → "• Pain: 1-2/10"), "• Pain: X/10" when single value (e.g., last procedure 3/10 → "• Pain: 1/10" or "• Pain: 2/10"), and omit the Pain bullet entirely when both pain scores are null. If \`overallPainTrend\` is "stable" or "worsened", render \`latestVitals\` pain directly without the -1/-2 delta. Use brackets for any other missing vital.
...
Clinical improvement summary. State sustained improvement following PRP treatment. Link pain reduction, functional restoration, and resolution of radicular features to favorable response to biologic regenerative therapy. REQUIRED: cite the numeric pain delta from baselinePain to the DISCHARGE-VISIT reading (1-2 points below \`latestVitals.pain_score_max\`), e.g., "a reduction from 7/10 at baseline to 1/10 at today's discharge evaluation". Note no treatment-related complications. Support stabilization and healing.
...
Favorable prognosis. Meaningful and sustained improvement in pain control, mobility, and functional capacity. Tie the outlook to the demonstrated numeric pain reduction across the treatment course, ending at the discharge-visit reading (1-2 points below \`latestVitals.pain_score_max\`). With continued adherence to conservative management, further improvement and long-term symptom control anticipated.
