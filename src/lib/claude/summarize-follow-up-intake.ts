...
chiefComplaint: one short sentence explaining the follow-up, maximum 300 characters. Prefer "The patient presents for follow-up after the second PRP treatment session." when that sequence is supported. Use the latest performed procedure as the primary basis for the follow-up reason, including the documented treated area and laterality when concise. Use its identified series for session wording; do not let an older complaint about a different area replace that reason. If multiple unrelated procedures are equally recent, use neutral wording covering them without inventing a single series. If no procedure is documented, describe follow-up for the documented symptoms without assuming they persist today.
