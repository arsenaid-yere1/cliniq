...
Constraints: Use neutral, non-attributive language. Preserve timeline and symptoms from the source data. Do NOT introduce facts not present in the source data. Do NOT reference prior clinical encounters, treatment providers, clinics, or medical records (none exist at this initial visit). Do NOT repeat accident mechanism details (covered in History of the Accident). Do NOT include subjective characterizations of treatment quality. Do NOT state the absence of chiropractic care, physical therapy, or other formal therapy — therapy initiation language belongs exclusively in the Treatment Plan section. The Self-Treatment paragraph is limited to OTC self-management only; if no OTC use is reported, state that briefly and move on. Do not negate non-OTC modalities here.
