...
describe('renderPrpTreatmentPlan', () => {
  ...
  it('renders validated ultrasound targets in the prior narrative template', () => {
    ...
    expect(text).toContain('Treatment will proceed in stages')
    expect(text).toContain('Any additional injection is not predetermined')
    expect(text).toContain('documented continued medical necessity and renewed informed consent')
