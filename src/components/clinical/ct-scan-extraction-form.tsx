...
import {
  ...
  type CtScanFinding,
} from '@/lib/validations/ct-scan-extraction'
import {
  approveCtScanExtraction,
  saveAndApproveCtScanExtraction,
  rejectCtScanExtraction,
} from '@/actions/ct-scan-extractions'

interface CtScanExtractionFormProps {
  extractionId: string
  defaultValues: CtScanReviewFormValues
  isManualEntry?: boolean
  onActionComplete: () => void
}

export function CtScanExtractionForm({
  extractionId,
  defaultValues,
  isManualEntry,
  onActionComplete,
  ...
  const form = useForm<CtScanReviewFormValues>({
    resolver: zodResolver(ctScanReviewFormSchema),
    defaultValues,
  })
  ...
  async function handleApprove() {
    ...
    const result = await approveCtScanExtraction(extractionId)
  ...
  async function handleSaveAndApprove(values: CtScanReviewFormValues) {
    setIsSubmitting(true)
    const result = await saveAndApproveCtScanExtraction(extractionId, values)
    setIsSubmitting(false)
    if (result.error) {
      toast.error(result.error)
    } else {
  ...
  async function handleReject() {
    ...
    setIsSubmitting(true)
    const result = await rejectCtScanExtraction(extractionId, rejectReason)
    setIsSubmitting(false)
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Textarea
                    rows={3}
                    placeholder="CT scan technique description..."
                    {...field}
                    value={field.value ?? ''}
                    onChange={(e) => field.onChange(e.target.value || null)}
                  />
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                variant="outline"
                size="sm"
                onClick={() => append({ level: '', description: '', severity: null } as CtScanFinding)}
              >
                ...
                <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      ...
                      <FormControl>
                        <Input placeholder="e.g. C5-C6" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
