...
import type { CtScanReviewFormValues, CtScanFinding } from '@/lib/validations/ct-scan-extraction'
...
export function CtScanExtractionReview({
  extraction,
  onBack,
}: {
  ...
  const overrides = extraction.provider_overrides as Partial<CtScanReviewFormValues> | undefined
  const findings = (Array.isArray(extraction.findings) ? extraction.findings : []) as CtScanFinding[]

  const defaultValues: CtScanReviewFormValues = {
    body_region: (overrides?.body_region ?? extraction.body_region) || '',
    scan_date: (overrides?.scan_date ?? extraction.scan_date) || null,
    technique: (overrides?.technique ?? extraction.technique) || null,
    reason_for_study: (overrides?.reason_for_study ?? extraction.reason_for_study) || null,
    findings: (overrides?.findings ?? findings) as CtScanFinding[],
    impression_summary: (overrides?.impression_summary ?? extraction.impression_summary) || null,
  }

  async function handleReExtract() {
    setIsReExtracting(true)
    const result = await extractCtScanReport(extraction.document_id)
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <h2 className="font-medium">
              {extraction.document?.file_name ?? 'CT Scan Extraction'}
            </h2>
