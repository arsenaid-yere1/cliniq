...
import { buildSeriesOptionLabel, getSeriesRelationshipDescription, START_SEPARATE_SERIES_LABEL, type ProcedureSeriesOption } from '@/lib/clinical/procedure-series-labels'
import type { ProcedureRecommendation } from '@/lib/validations/pain-follow-up-note'

export function ProcedureOrderDialog({caseId,episodeId,encounterId,recommendation,seriesOptions=[]}:{caseId:string;episodeId:string;encounterId:string;recommendation:ProcedureRecommendation;seriesOptions:ProcedureSeriesOption[]}) {
  ...
  const matchingOptions=seriesOptions.filter((option)=>option.procedureType===recommendation.procedure_type)
  const selectedOption=matchingOptions.find((option)=>option.id===continuation)
