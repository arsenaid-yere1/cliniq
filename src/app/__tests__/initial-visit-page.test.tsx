...
const state = vi.hoisted(() => ({ vitals: vi.fn() }))
vi.mock('@/actions/initial-visit-notes', () => ({
  ...
  getProviderIntake: async () => ({ data: null }),
}))
...
const renderPage = async () => await InitialVisitPage({ params: Promise.resolve({ caseId: 'case' }), searchParams: Promise.resolve({ episode: 'episode-2' }) }) as ReactElement<{ role?: string; initialVitals?: unknown; canGenerate?: boolean }>
